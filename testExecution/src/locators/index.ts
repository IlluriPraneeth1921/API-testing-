 ...
 * - table.locators.ts     → Blue Compass specific column names & table actions
 *                           (generic mat-table/paginator → use CommonLocators)
 * - forms.locators.ts     → Blue Compass specific form fields & validation messages
 *                           (generic inputs/buttons → use CommonLocators)
 * - search.locators.ts    → Search functionality
 * - login.locators.ts     → Login page specific
 * - bulk-assignment.locators.ts → Bulk Assignment screen specific
 * - person-profile.locators.ts  → Person Profile screen specific
 *
 * ─── LOCATOR HIERARCHY (April 2026 refactor) ──────────────────────────────────
 *
 *   CommonLocators          → Angular Material base + generic pagination/buttons
 *   TableLocators           → Blue Compass column names (.mat-column-*), HTML table
 *   FormLocators            → Blue Compass form fields, validation, page headers
 *   <Screen>Locators        → Screen-specific aria-labels, titles, id patterns
 *
 * RULE: If a selector is generic Angular Material → CommonLocators.
 *       If it has a screen-specific aria-label/title/id → screen locator file.
 *       Run `npm run audit:locators` to check for duplicates.
 *
 * ──────────────────────────────────────────────────────────────────────────────
 *
...
export { PersonProfileLocators } from './person-profile.locators';
...
export type { PersonProfileLocatorKeys } from './person-profile.locators';
