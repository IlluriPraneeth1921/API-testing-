...
export class BulkAssignmentListPage {
  ...
  async clickAssignStaff() {
    ...
    await this.page.locator(BulkAssignmentLocators.assignStaffBtn).click({ force: true });
    await this.page.waitForTimeout(Waits.MIN);
  }
  ...
  /**
   * Capture a snapshot of current grid row texts for later comparison.
   * Returns array of row text content (trimmed).
   */
  async captureGridSnapshot(): Promise<string[]> {
    ...
    console.log(`[BA_LIST] Grid snapshot captured: ${count} rows`);
    return snapshot;
  }

  /**
   * Verify the current grid matches a previously captured snapshot (data unchanged).
   */
  async verifyGridMatchesSnapshot(snapshot: string[]): Promise<boolean> {
    const current = await this.captureGridSnapshot();
    const matches = JSON.stringify(current) === JSON.stringify(snapshot);
    console.log(`[BA_LIST] Grid vs snapshot: ${matches ? '✓ unchanged' : '✗ changed'}`);
    return matches;
  }

  /**
   * Verify the current grid differs from a previously captured snapshot (data updated).
   */
  async verifyGridDiffersFromSnapshot(snapshot: string[]): Promise<boolean> {
    const current = await this.captureGridSnapshot();
    const differs = JSON.stringify(current) !== JSON.stringify(snapshot);
    console.log(`[BA_LIST] Grid vs snapshot: ${differs ? '✓ data updated' : '✗ still same'}`);
    return differs;
  }

  /**
   * Verify the "Select All Records" button is NOT visible anywhere on the page.
   * PBI 915981: This button should never appear — only per-page selection is allowed.
   */
  async verifySelectAllButtonNotVisible(): Promise<void> {
    const btn = this.page.locator('button:has-text("Select All Records"), button:has-text("Select All")').first();
    const visible = await btn.isVisible({ timeout: Waits.MIN }).catch(() => false);
    if (visible) {
      console.log('[BA_LIST] ✗ "Select All Records" button IS visible — this is a bug');
    } else {
      console.log('[BA_LIST] ✓ "Select All Records" button is NOT visible (expected)');
    }
    await expect(btn).not.toBeVisible({ timeout: Waits.MIN });
  }

  /**
   * Scroll the grid to the bottom to check for any hidden elements.
   */
  async scrollGridToEnd(): Promise<void> {
    const table = this.page.locator(CommonLocators.matTable).first();
    if (await table.isVisible({ timeout: Waits.MIN }).catch(() => false)) {
      await table.evaluate(el => el.scrollTop = el.scrollHeight);
      await this.page.waitForTimeout(Waits.MIN);
    }
    console.log('[BA_LIST] Scrolled grid to end');
  }

  /**
   * Scroll the grid back to the top.
   */
  async scrollGridToTop(): Promise<void> {
    const table = this.page.locator(CommonLocators.matTable).first();
    if (await table.isVisible({ timeout: Waits.MIN }).catch(() => false)) {
      await table.evaluate(el => el.scrollTop = 0);
      await this.page.waitForTimeout(Waits.MIN);
    }
    console.log('[BA_LIST] Scrolled grid to top');
  }
}
