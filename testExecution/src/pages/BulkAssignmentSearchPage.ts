...
export class BulkAssignmentSearchPage {
  ...
  /**
   * Open the Advanced Search panel WITHOUT resetting the form.
   * Use this when you want to re-run the same search criteria (e.g. after a bulk operation).
   * If the panel is already open, does nothing.
   * If the panel is closed, opens it — criteria filled previously are preserved by the app.
   */
  async openPanelPreservingCriteria() {
    const runQueryBtn = this.page.locator(BulkAssignmentLocators.runQueryBtn).first();
    const alreadyOpen = await runQueryBtn.isVisible({ timeout: Waits.MIN }).catch(() => false);
    if (alreadyOpen) {
      console.log('[BA_SEARCH] Panel already open — criteria preserved');
      return;
    }
    console.log('[BA_SEARCH] Opening panel (preserving existing criteria)');
    await this.page.locator(BulkAssignmentLocators.advancedSearchBtn).click({ force: true });
    await this.page.waitForTimeout(Waits.MAX); // wait longer — panel animation + criteria re-render
    await expect(runQueryBtn).toBeVisible({ timeout: Waits.ELEMENT_TIMEOUT });
  }
